function chechVersion() {
    try {
        var version = otaku.getEngineVersion();
        if (version == null || version == "" || version < 2) {
            throw "版本过低";
        }
    } catch (e) {
        throw "需要更高版本的内核，请升级APP";
    }
}
chechVersion();
function home_parse(url, html) {
    ...
    var list = [];
    data['UpdateComicItems'].forEach(function (e, i) {
        var aggregate = {};
        aggregate.title = e['Title'];
        aggregate.cover = e['ShowConver'];
        aggregate.link = 'http://mangabz.com/' + e['UrlKey'];
        aggregate.info = '最新：' + e['ShowLastPartName'] + '\t介绍：' + e['Content'];
        aggregate.updateTime = e['LastUpdateTime'];
        list.push(aggregate);
    });
